namespace TheTankGame.Core
{
    ...
    public class TankManager : IManager
    {
        ...
        public string AddVehicle(IList<string> arguments)
        {
            ...
            IVehicle vehicle = this.vehicleFactory.CreateVehicle(vehicleType,
                model,
                weight,
                price,
                attack,
        ...
        public string AddPart(IList<string> arguments)
        {
            ...
            IPart part = this.partFactory.CreatePart(partType,
                model,
                weight,
                price,
                additionalParameter);

            if (part != null && this.vehicles.ContainsKey(vehicleModel))
            {
                this.parts.Add(part.Model, part);
                this.vehicles[vehicleModel].GetType()
                    .GetMethods()
                    .FirstOrDefault(m => m.Name.Contains(part.GetType().Name))
        ...
        //check to string method if bugs are present
